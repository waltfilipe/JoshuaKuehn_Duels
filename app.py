...
def get_style(event_type):
    if "WON" in event_type: return 'o', (0, 0.6, 0, 0.8), 90, 0.5
    if "LOST" in event_type: return 'x', (0.9, 0, 0, 0.9), 90, 2.5
    if "FOULED" in event_type: return 's', (1, 0.5, 0, 0.8), 90, 0.5
...
with col_map:
    ...
    for _, row in df.iterrows():
        ...
        ec = 'black' if row["video"] else color
    ...
    legend_elements = [
        ...
        Line2D([0], [0], marker='x', color=(0.9, 0, 0), label='Lost', markersize=8, markeredgewidth=2),
...
with col_vid:
    ...
    total_duels = len(df[df['type'].str.contains('DUEL|AERIAL')])
    wins = len(df[df['type'].str.contains('WON')])
    fouls = len(df[df['type'] == 'FOULED'])
    win_rate = (wins / total_duels * 100) if total_duels > 0 else 0

    s_col1, s_col2, s_col3 = st.columns(3)
    s_col1.metric("Total Duels", total_duels)
    s_col2.metric("Success Rate", f"{win_rate:.1f}%")
    s_col3.metric("Fouls Won", fouls)
